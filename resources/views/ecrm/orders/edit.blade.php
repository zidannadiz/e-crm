        ...
        <form action="{{ route('ecrm.orders.update', $order) }}" method="POST">
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <textarea name="deskripsi" rows="5" required class="w-full border rounded px-4 py-2">{{ old('deskripsi', $order->deskripsi) }}</textarea>
                    @error('deskripsi') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Kebutuhan Tambahan</label>
                    <textarea name="kebutuhan" rows="3" class="w-full border rounded px-4 py-2">{{ old('kebutuhan', $order->kebutuhan) }}</textarea>
                ...
                <div>
                    ...
                    <input type="date" name="deadline" value="{{ old('deadline', $order->deadline ? $order->deadline->format('Y-m-d') : '') }}" class="w-full border rounded px-4 py-2">
                ...
                <div>
                    ...
                    <select name="produk_status" required class="w-full border rounded px-4 py-2">
                ...
                <div>
                    ...
                    <input type="number" name="budget" step="0.01" value="{{ old('budget', $order->budget) }}" class="w-full border rounded px-4 py-2" placeholder="0.00">
                    @error('budget') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Catatan Admin</label>
                    <textarea name="catatan_admin" rows="3" class="w-full border rounded px-4 py-2">{{ old('catatan_admin', $order->catatan_admin) }}</textarea>
