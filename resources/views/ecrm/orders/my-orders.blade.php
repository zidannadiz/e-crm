...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        @forelse($orders as $index => $order)
            <div class="bg-white rounded-lg shadow p-6 animate-slide-up hover:shadow-lg transition-all duration-300 hover:scale-105" style="animation-delay: {{ $index * 0.1 }}s; opacity: 0; animation-fill-mode: forwards;">
                <div class="flex justify-between items-start mb-4">
                    ...
                    <div class="text-right space-y-1">
                        ...
                        @endphp
                        <span class="inline-flex px-5 py-1 text-sm font-semibold rounded-full transition-all duration-200 hover:scale-110 {{ 
                ...
                <div class="flex gap-2">
                    <a href="{{ route('ecrm.orders.show', $order) }}" class="flex-1 bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 text-center text-sm transition-all duration-200 hover:scale-105 active:scale-95 ripple">
                        Lihat Detail
                    </a>
                    <a href="{{ route('ecrm.chat.index', $order) }}" class="flex-1 bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700 text-center text-sm transition-all duration-200 hover:scale-105 active:scale-95 ripple">
                        Chat
                    </a>
                </div>
                ...
                <div class="flex gap-2 mt-2">
                    <a href="{{ route('ecrm.orders.edit', $order) }}" class="flex-1 bg-gray-800 text-white px-4 py-2 rounded hover:bg-gray-900 text-center text-sm flex items-center justify-center gap-1 transition-all duration-200 hover:scale-105 active:scale-95 ripple">
                    ...
                    <form action="{{ route('ecrm.orders.destroy', $order) }}" method="POST" class="flex-1 delete-form" data-message="Apakah Anda yakin ingin menghapus pesanan ini? Tindakan ini tidak dapat dibatalkan.">
                        @csrf
                        @method('DELETE')
                        <button type="button" class="w-full bg-red-600 text-white px-4 py-2 rounded hover:bg-red-700 text-center text-sm flex items-center justify-center gap-1 delete-btn transition-all duration-200 hover:scale-105 active:scale-95" style="cursor: pointer;">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
