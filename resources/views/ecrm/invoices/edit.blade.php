        ...
        <form action="{{ route('ecrm.invoices.update', $invoice) }}" method="POST" id="invoiceForm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="md:col-span-2">
                    ...
                    <select name="order_id" id="order_id" required class="w-full border rounded px-4 py-2">
                ...
                <div>
                    ...
                    <input type="date" name="tanggal_invoice" value="{{ old('tanggal_invoice', $invoice->tanggal_invoice->format('Y-m-d')) }}" required class="w-full border rounded px-4 py-2">
                ...
                <div>
                    ...
                    <input type="date" name="tanggal_jatuh_tempo" value="{{ old('tanggal_jatuh_tempo', $invoice->tanggal_jatuh_tempo->format('Y-m-d')) }}" required class="w-full border rounded px-4 py-2">
                    @error('tanggal_jatuh_tempo') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Subtotal *</label>
                    <input type="number" name="subtotal" id="subtotal" value="{{ old('subtotal', $invoice->subtotal) }}" step="0.01" required class="w-full border rounded px-4 py-2" oninput="calculateTotal()">
                ...
                <div>
                    ...
                    <input type="number" name="pajak" id="pajak" value="{{ old('pajak', $invoice->pajak) }}" step="0.01" class="w-full border rounded px-4 py-2" oninput="calculateTotal()">
                ...
                <div>
                    ...
                    <input type="number" name="diskon" id="diskon" value="{{ old('diskon', $invoice->diskon) }}" step="0.01" class="w-full border rounded px-4 py-2" oninput="calculateTotal()">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="deskripsi" rows="3" class="w-full border rounded px-4 py-2">{{ old('deskripsi', $invoice->deskripsi) }}</textarea>
                    @error('deskripsi') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>

                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-2">Catatan</label>
                    <textarea name="catatan" rows="3" class="w-full border rounded px-4 py-2">{{ old('catatan', $invoice->catatan) }}</textarea>
